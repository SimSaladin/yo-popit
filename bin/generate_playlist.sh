...
if [[ ! -s "$HERE/$SILENCE_FILE" ]]; then
   echo "FATAL: no silence file"
   exit
fi
...
construct_playlist(){
   ...
   echo "$items" \
      | cut -d' ' -f 2- \
      | xargs -d'\n' -I{} realpath {} \
      | tee "$HERE/playlists/playlist.m3u"

   echo
   echo "DONE Playlist written as "$HERE/playlists/playlist.m3u". Move it appropriately."
   echo
